...
- **Non-owners check back, while they have a local player.** A replica that registered a bot game but does not own it tries ownership again every `OWNERSHIP_LEASE_TTL_SEC` (the value `serve.ts` passes as `nonOwnerRecheckMs`). This is what recovers a game whose owner died, since nothing is broadcast when a lease expires. A re-check is one ownership attempt and no log read; a claim that succeeds reloads through the router's takeover path, which also reveals a game that ended out of sight. It never makes an engine call unless the claim succeeds.
- **Non-owners let go when their last local session leaves.** `RealtimeGateway` now reports when a game's last session on a node leaves (`onGameUnloaded`, the pair of `onGameLoaded`), and the mover unregisters the game there unless `OwnershipRegistry` still lists it as this node's (`claimedHere`, true even while a renewal is failing). A non-owner is registered only to take over for a local player, so this bounds its re-checks, its per-broadcast ownership attempts and anything a lost `ended` broadcast could leave behind to the time someone on that replica is in the game. The owner keeps its registration: the player may have reconnected through another replica while this node keeps renewing the lease, and nobody else would move for the bot.
