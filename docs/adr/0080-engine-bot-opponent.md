...
- **Reload debt belongs to a claim.** The router's takeover reload (`rehydrateIfStale`, shared with `route()`) was keyed by game only, so a reload begun for an earlier claim, whose log read predated another owner's moves, could finish after a later claim and settle that claim's debt, leaving a stale copy marked fresh. Each claim now gets a number from one process-wide counter; only a reload started for the current claim clears the debt, and a caller that finds an older reload in flight starts a new one, which, for a game in this node's cache, queues behind the older one on the game's command lock in `GameAuthority.reloadFromLog` and so reads the log after it. (For a game not in the cache, `reloadFromLog` reads without the lock. Every path here starts from a cached game: a mover registers only after a join loads it, commands come from joined sessions, and nothing in production evicts a live game.) Numbers are never reused, so the entry is dropped once settled.
...
- **Non-owners check back, while they have a local player.** A replica that registered a bot game but does not own it re-checks every `OWNERSHIP_LEASE_TTL_SEC` (the value `serve.ts` passes as `nonOwnerRecheckMs`): it reloads its copy from the event log, unregisters the game if it has ended, and otherwise tries ownership again. This is what recovers a game whose owner died, since nothing is broadcast when a lease expires, and a game whose `ended` broadcast was lost, since Redis pub/sub is best-effort. The reload is a full replay of the game (quadratic in its length), which is why re-checks run only while someone on that replica is in the game — apart from a pass that fails outright (Redis or the event log unreachable), which schedules a re-check regardless, since retrying is how it recovers; the next pass that succeeds applies the rules above. It never makes an engine call unless the claim succeeds.
- **Non-owners let go after their last local session leaves.** `RealtimeGateway` now reports when a game's last session on a node leaves (`onGameUnloaded`, the pair of `onGameLoaded`). Whether this node is the owner cannot be read off a snapshot at that moment, because a claim may be in flight, so the mover only records that no local player is left and takes a pass: the game is dropped when, and only when, a pass fails to own it, and a join clears the mark. A non-owner therefore lets go right after its sessions leave, a claim that was in flight and succeeds keeps the game, and a former owner lets go on its first failed claim after losing the game. The owner keeps its registration: the player may have reconnected through another replica while this node keeps renewing the lease, and nobody else would move for the bot. This bounds re-checks, per-broadcast ownership attempts and anything a lost `ended` broadcast could leave behind to the time someone on that replica is in the game.
...
- Remaining limits: a dead owner's game resumes after its lease expires plus up to one re-check period (at most about twice the lease TTL), and only on a replica where someone is in the game; with nobody on any surviving replica it waits for the next join, which registers it. A lease that keeps lapsing while the engine thinks costs one engine call per lapse, since each dropped result is followed by a fresh pass. While a non-owner has a local session, every broadcast costs it an ownership attempt (two Redis round trips), which is unavoidable while its copy is stale, and every lease period one full reload. An owner with no local session keeps its registration until the game ends or it loses the game; it makes no re-checks unless a pass fails outright. `route()` itself does not re-check the lease after its takeover reload the way `prepareOwnership` does; that predates this amendment and is left for a follow-up.

`services/gateway/test/engine-bot-multinode.integration.test.ts` proves this against the production router, registry and per-node Redis pub/sub on real Redis: only the owner calls the engine, including on duplicate broadcasts; Bot-as-White at ply 0 gets exactly one claimant; a takeover computes from the rehydrated position; a result is not applied after ownership is lost, or lost and regained, while thinking; a reload begun for an earlier claim does not settle a later one; a lease that lapsed unnoticed and is then re-claimed still reloads; an owner that dies is replaced with no further event; taking over a game that ended out of sight unregisters it after the reload without an engine call; a finished game stops every replica. It also shows that when local sessions leave, the non-owner lets go while the owner keeps moving the bot for a player who moves through the other replica. `engine-bot.test.ts` covers the lapsed-lease re-run, the orphaned-game re-check, the missed-`ended` re-check, the session-leave rule (a claim in flight when the last session leaves, a former owner, a rejoin) and the finished-game join, and `packages/realtime-gateway/test/gateway.test.ts` the `onGameUnloaded` hook. Fifteen deliberate defects were each introduced alone, against the compiled test build: disabling the pre-engine ownership gate, the post-engine check, the `ended` unregister, the reload-debt clause of `holdsOwnership`, the claim-generation check, the re-run after a dropped result, the non-owner re-check, the reload before a re-check pass, the unregister of a finished copy before ownership, the unregister of a game the post-claim reload shows finished, the new-claim rule for a `SET NX` that creates the key and the gateway's unload hook; and, for the session-leave rule, never letting go, letting go at once regardless of the claim, and a join not clearing the mark. Each disabling failed at least one of these tests.
