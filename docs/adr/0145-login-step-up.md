...
- **Verification re-send.** A correct password on an unverified account re-sends the verification email at most once every 10 minutes. `POST /v1/auth/email/verification/resend` does the same without a session, for a handle or email: it always answers `202`, sends only to an existing unverified address, and is limited per IP only, because a per-handle bucket would let anyone stop the owner's new link. A re-send on either path adds a link and leaves earlier unused ones valid for their 24 hours; superseding them would let anyone who knows the handle invalidate the owner's link on demand. The route answers before it looks anything up and does the work in the background, so its timing cannot tell an unverified account from an unknown or verified one. This is how an unverified owner gets back in while their handle is in step-up, where the password alone must not reveal that the address is unverified.
- **Undelivered email.** Email is sent as tracked background work, so a request takes the same time whether or not a message went out. When the provider says definitively that it did not send (it rejected or throttled the message), that sign-in code or verification link is discarded, so it neither blocks a new code nor counts toward the re-send cooldown. A timeout or an unreadable provider answer may still have delivered it, so its token stays valid and the owner waits out the code or cooldown at worst. A failed discard is logged with the request and trace ids and the stack, but no address, user, token or code. Graceful shutdown drains this background work, with a bound, before the database pool closes.
...
- An attacker who *already knows* the password still cannot sign in without the mailbox, but can spend each code with wrong guesses, which delays an owner who has no passkey. The cooldown bounds the emails this forces (about twelve an hour). The owner's remedy is a password reset, which proves the mailbox, replaces the leaked password and revokes sessions; the code email says so. At the time of this decision, the password-reset request endpoint kept its own per-target limit outside this change; ADR-0146 replaces that unsafe limit with one-live-token issuance.
- An account whose email is not yet verified gets no code in step-up mode, only the uniform `step_up_required` answer; its owner re-sends the verification link without a session and verifies first. Answering with `details.reason: email_unverified` there instead would tell a guesser that the password was right.
- Pre-launch password accounts created without an email cannot sign in with the password and have no address to verify. Rookzen has no production users, so the owner chose not to build a migration for them (they can still use a passkey).
