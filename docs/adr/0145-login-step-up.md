...
- **Undelivered email.** Email is sent in the background, so a request takes the same time whether or not a message went out. If the provider reports a sign-in code or a verification link undelivered, that token is discarded in the background, so it neither blocks a new code nor counts toward the re-send cooldown. A failed discard is logged (purpose and error only, no address, user, token or code).
