...
- **Verification re-send.** A correct password on an unverified account re-sends the verification email at most once every 10 minutes. `POST /v1/auth/email/verification/resend` does the same without a session, for a handle or email: it always answers `202`, sends only to an existing unverified address, and is limited per IP only, because a per-handle bucket would let anyone stop the owner's new link. This is how an unverified owner gets back in while their handle is in step-up, where the password alone must not reveal that the address is unverified.
