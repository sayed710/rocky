...
- Past an account-wide failure threshold per handle (10 per 15 minutes, counted by the new `RateLimiter.tally`, which signals and never refuses), a password alone no longer signs in. The owner uses a passkey, or sends the password with an 8-digit code emailed to the account's verified address. A code is issued only for a correct password on a verified account. It lasts 10 minutes, is single-use, and dies after 5 wrong codes presented with the correct password. At most one is live per account, and a live one is never replaced. Guesses without the password cannot trigger an email or spend the owner's code. Every step-up failure is the same `401` with `details.reason: step_up_required`, whatever the handle, password or code. The code statements always run, with a decoy id for unknown handles.
- Public registration requires an email, and a password account cannot sign in with its password until that email is verified. A correct password on an unverified account re-sends verification and answers `403` with `details.reason: email_unverified`. WebAuthn login options are limited per IP only; challenges stay random, single-use and bound server-side, and a flood cannot evict the owner's challenge.
...
- Migration 0037 adds the `login_step_up` token kind, an attempt counter and a one-live-code index (ADR-0145). The web sign-in form shows a code field only after a step-up answer, and registration refuses a blank email before sending. The e2e harness records outgoing email (`GET /e2e/outbox`) so specs can verify an account as its owner would.
