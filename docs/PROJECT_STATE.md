...
- A backoff never hides a peer logout: `refreshNow()` applies a persisted logout barrier before consulting it. Optional-auth requests (for example `GET /v1/seeks`) go out anonymously during a transient refresh outage instead of failing. An explicit logout ignores the backoff and makes one real refresh attempt, so a recovered server can still revoke the refresh session; if the server stays unreachable, only the local session is cleared, as on `main`.
