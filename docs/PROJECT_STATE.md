...
- Recovery without events: a replica that registered a bot game but does not own it re-checks every lease TTL, reloading its copy from the event log, so a game whose owner died is claimed and continued, and a game whose `ended` broadcast was lost is unregistered. A copy that already shows the game over unregisters before any ownership traffic, which covers joining a finished game on a non-owner. Remaining limits: a dead owner's game resumes after its lease expiry plus up to one re-check period, and only where some replica has it registered; each non-owner registration costs a full reload per lease TTL for the game's lifetime. The chart refuses a non-boolean `gateway.engineBot.enabled`, since a quoted `"false"` would leave the bot on.
