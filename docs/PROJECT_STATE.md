...
- The per-IP bucket still charges every attempt before credential verification. Failed attempts now count against two failure budgets keyed by the submitted handle: per handle and source address (5 per 15 minutes, the old per-handle rate), and per handle across all addresses (50 per 15 minutes). Both reserve a slot in the same atomic admission as the IP bucket, so concurrent guesses cannot overshoot a nearly full bucket. `RateLimiter.refund` hands the slots back when the password is correct, so a successful login costs no failure budget. One attacker address exhausts only its own budget for a handle. Blocking the owner now takes failures from at least ten distinct addresses per window, and passkey login is not affected by it. This is a deliberate trade-off. One address still gets the old 5 guesses per window against a handle. A distributed attacker gets up to 50 across all addresses, up from 5, and that is the cost of no longer letting a single address lock the owner out. Unknown and real handles go through identical buckets, so status, body, and `Retry-After` still match.
- `/v1/auth/webauthn/login/options` charged a per-handle bucket for each challenge request. That allowed the same lockout and protected nothing: challenges are random, single-use, keyed by their own hash (so a flood cannot evict the owner's challenge), and a passkey signature cannot be guessed. The route is now limited per IP only, and `rateLimit.webauthnLogin.perHandle` is removed. Challenge binding, single-use consumption, and replay refusal are unchanged, and a test covers them after a flood.
- Admission returns a reservation, naming the key and the charged window, for each bucket marked `refundable`. `RateLimiter.refund` takes those reservations and acts only while that same window is current and live. A login that outlives its window therefore cannot hand a slot back to the next window, which other requests have charged. A reservation is also single-use and valid only on the limiter that issued it; each limiter tracks its outstanding reservations in-process. A replayed or forged reservation therefore refunds nothing, and cannot take back a later request's charge. In PostgreSQL the window identity is its start in whole microseconds. Refunds run in the same sorted-key transaction as multi-bucket admission, with the same lock and statement timeouts. Each refund locks the row with `FOR UPDATE`, judges liveness with `clock_timestamp()` so time spent waiting on the lock counts, then decrements the row or deletes the last unit, which respects `CHECK (request_count > 0)`. Login logs a refund fault through the request-scoped logger and does not throw it. By then a session exists, and the slot just stays charged until its window ends, which fails closed.
- Scope: API login throttling, the rate-limiter port, and their tests. No migration. This PR does not touch terminal-outcome recovery, the realtime gateway, persistence, tournaments, or messaging budgets.
