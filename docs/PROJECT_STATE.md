...
- Source classification from the creation paths on `main`: seek acceptance and `DurableGameLauncher` create two-human games and now write `GameCreated.source` (`seek` or `tournament`) together with `noShowAfterMs`, the game's own durable deadline from `NO_SHOW_SEEK_MS` / `NO_SHOW_TOURNAMENT_MS` (defaults 60 000 and 300 000, validated positive integers). `POST /v1/games/bot` (human vs a first-party engine that never joins) and `GameAuthority.createGame` (used only by the e2e harness and tests) write neither and keep the original lifecycle unchanged. Stored games replay and continue exactly as before.
- Readiness: a new `PlayerReady` event, emitted by `Game.markReady` only for a sourced game that is ongoing, has no move, is before its deadline and whose seat is not yet ready, so repeats and late joins are no-ops. After an authenticated seated join, `RealtimeGateway` routes a `ready` command to the game's owner through the command router, where it is serialized with moves and appended with `expectedSeq`; a losing append is re-routed (up to three attempts) against the reloaded log. Spectators, anonymous connections and other users' tokens never route it, and the wire decoder accepts neither `ready` nor `expireNoShow`. Readiness survives disconnects and restarts because it is only ever read from the log.
...
- No-show: seek games end `'*'`/`no_show` at their deadline if no move was played, whoever was ready; tournament games end as a forfeit win for the one player ready before the deadline or a winnerless double forfeit, and wait indefinitely when both were ready in time. The owner enforces the deadline on every command: a move or join arriving after it records the no-show instead, like a move after a flag fall. `NoShowExpiryWorker` runs on every gateway with `DATABASE_URL` for games nobody touches: it reads due entries of `pregame_deadlines`, re-decides each from the log, routes `expireNoShow` (which carries no deadline) as `NO_SHOW_ACTOR` to the owner, dismisses games that will never expire, and when nobody on the replica is watching releases a lease claimed only for the expiry and evicts a copy loaded only for it. The owner's command lock plus the log's sequence check make a first move and an expiry mutually exclusive and make a second expiry a no-op. Shutdown awaits the in-flight pass before Redis and the pool close.
...
- Storage: migration 0042 adds the `no_show` termination and the `pregame_deadlines` queue, kept by a `game_events` insert trigger in the same transaction as every append (creation with a deadline enters; first move or ending leaves), so it cannot lag the log or miss a game written by another release. The games projection needs no new column: `PlayerReady` advances `last_seq` but not the ply count, and a no-show ending projects result, `termination = 'no_show'` and `ended_at`, which also makes the ended-game seek receipt guard truthful. The pre-0040 projector test now migrates through 0041 for its count assertion.
- Protocol and UI: `StateView.ready` (null for games without a source), a `ready` broadcast on the game channel (replayed on resume), and the `not_ready` reject code. The web client reads a missing or malformed `ready` as none (keeping what it knew when a snapshot from an older gateway lacks the field), merges broadcasts monotonically, keeps the board closed while a seat is not ready, and shows waiting and no-show states. No visual change.
- CI: the gateway-service job now also has a pgvector PostgreSQL service and `DATABASE_URL`, because the routed-replica suite needs both Redis and PostgreSQL; `scripts/ci-local.mjs` requires both for that job.
