...
- Recovery without events: a replica that registered a bot game but does not own it tries ownership every lease TTL (no log read; a successful claim reloads through the router), so a game whose owner died is claimed and continued. `RealtimeGateway` gains `onGameUnloaded` (the last session in a game left this node); a non-owner then unregisters, which bounds its re-checks and anything a lost `ended` broadcast could leave behind, while the owner keeps the game for a player who reconnected elsewhere. A copy that already shows the game over unregisters before any ownership traffic, which covers joining a finished game on a non-owner. Remaining limit: a dead owner's game resumes after its lease expiry plus up to one re-check period, and only where someone is in the game on a surviving replica. The chart refuses a non-boolean `gateway.engineBot.enabled`, since a quoted `"false"` would leave the bot on.
