...
- `refund` never takes a bucket below zero and never touches a lapsed window. In PostgreSQL it runs in the same sorted-key transaction as multi-bucket admission, with the same lock and statement timeouts. It locks each live row with `FOR UPDATE`, then decrements it or deletes the last unit, which respects `CHECK (request_count > 0)`. Login logs a refund fault and does not throw it: by then a session exists, and the unrefunded slot just stays charged until its window ends, which fails closed. `RouteDeps` now carries the resolved logger for this. A structural test requires login to refund only its two failure buckets, and only after `auth.login`. Deterministic in-memory route tests, and real-PostgreSQL tests across two limiter instances, cover the attack, the owner's login, per-IP limiting, enumeration parity, success without cost, concurrent reservations, refund races, the refund lock timeout, and the owner's recovery once a distributed attack's window ends.
