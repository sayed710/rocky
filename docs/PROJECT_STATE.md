...
- Tests: `engine-bot.test.ts` adds the ownership gate, post-engine checks, lapsed-lease re-run, re-check recovery and finished-game cleanup with a scripted gate; `engine-bot-multinode.integration.test.ts` proves the multi-replica behaviour on real Redis with the production router, registry and per-node Redis pub/sub (owner-only computation including duplicate broadcasts, ply-0 claim, takeover rehydration, ownership lost or lost-and-regained mid-think, an old reload landing after a later claim, a dead owner replaced with no further event, terminal stop). Its Redis clients keep production retry settings; a test-only preflight fails the file in seconds, by name, when Redis is unreachable. Nine safety lines were each disabled in turn, one at a time against the compiled test build, and each disabling failed at least one test. `helm-snapshot-test.sh` pins the flag on, off, per strategy, absent from the search indexer, and the rest of the gateway env contract including the tournament reporter.
