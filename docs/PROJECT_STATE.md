...
- Public registration requires an email, and a password account cannot sign in with its password until that email is verified. A correct password on an unverified account re-sends verification (at most every 10 minutes) and answers `403` with `details.reason: email_unverified`. `POST /v1/auth/email/verification/resend` re-sends the link without a session (always `202`, per-IP limit only, same 10-minute cooldown, and earlier links stay valid so nobody can invalidate the owner's link), which is how an unverified owner recovers while their handle is in step-up. A code the email provider reports undelivered is discarded in the background, so the next attempt issues a fresh one. WebAuthn login options are limited per IP only; challenges stay random, single-use and bound server-side, and a flood cannot evict the owner's challenge.
