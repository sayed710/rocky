...
_Last updated: 2026-09-26 — M15 Increment 71: Durable player readiness, first-move clock start and source-specific no-show._

Prior: _Last updated: 2026-09-26 — M15 Increment 70: Durable event-log-driven games projection._

Prior: _Last updated: 2026-09-26 — M15 Increment 69: Owner-only engine bot decisions and Helm ENGINE_BOT parity._
...
## M15 Increment 67 — Login step-up instead of account lockout (2026-09-25)

- Audit P1-1. `/v1/auth/login` charged a per-handle bucket (5 per 15 minutes) on every attempt, before checking the password, so anyone who knew a handle could keep its owner out. Reproduced on `main`: five wrong guesses from an attacker's address got the owner a 429 for the correct password from their own address. `/v1/auth/webauthn/login/options` had the same shape, a per-handle bucket for merely requesting a challenge.
- Password login now admits against the per-IP bucket (every attempt, before credential work) and a per-handle-and-source failure budget (5 per 15 minutes). The budget's slot is reserved at admission and refunded only when a session is issued. There is no account-wide bucket that refuses: an intermediate revision kept a 50-failure cap, and ten addresses could fill it and refuse the owner, which the owner rejected.
- Past an account-wide failure threshold per handle (10 per 15 minutes, counted by the new `RateLimiter.tally`, which signals and never refuses), a password alone no longer signs in. The owner uses a passkey, or sends the password with an 8-digit code emailed to the account's verified address. A code is issued only for a correct password on a verified account. It lasts 10 minutes, is single-use, and dies after 5 wrong codes presented with the correct password. At most one is live per account. A live code is never replaced, and an exhausted one is replaced only 5 minutes after it was issued, so someone who already knows the password cannot flood the owner's inbox. Codes are stored as an HMAC under a key derived from the access-token secret, not as a plain hash of eight digits. Guesses without the password cannot trigger an email or spend the owner's code. Every step-up failure is the same `401` with `details.reason: step_up_required`, whatever the handle, password or code. The code statements always run, with a decoy id for unknown handles.
- Public registration requires an email, and a password account cannot sign in with its password until that email is verified. A correct password on an unverified account re-sends verification (at most every 10 minutes) and answers `403` with `details.reason: email_unverified`. `POST /v1/auth/email/verification/resend` re-sends the link without a session (always `202`, per-IP limit only, same 10-minute cooldown, and earlier links stay valid so nobody can invalidate the owner's link), which is how an unverified owner recovers while their handle is in step-up. The public re-send answers before any lookup and does its work in the background, so timing cannot reveal an unverified account. A code or verification link the email provider definitively refused (rejected or throttled) is discarded in the background, so it neither blocks a new code nor counts toward the cooldown; a timeout keeps the token, because the message may have arrived. Background auth work is tracked and drained on graceful shutdown before the pool closes, and its failures are logged with request correlation and stack but no identifying data. WebAuthn login options are limited per IP only; challenges stay random, single-use and bound server-side, and a flood cannot evict the owner's challenge.
- Rate-limit reservations are bound to the window that took the charge and are single-use per issuing limiter, so a late or replayed refund cannot take back another request's charge. `PgRateLimiter` refunds run in the same sorted-key, lock- and statement-bounded transaction as multi-bucket admission. A refund fault after a successful login is logged through the request logger and does not fail the login.
...
- `EngineBotMover` now asks the engine only on the owning replica. Before computing it calls `RedisCommandRouter.prepareOwnership` (valid lease, or claim, then the existing takeover reload), so an unowned game (bot as White at ply 0, an owner gone) is claimed and rehydrated first, and a non-owner returns without an engine call. After computing it submits only if `holdsOwnership` is still true (valid lease, no reload debt) and the bot is still to move at the same ply and FEN. Otherwise the result is dropped and the mover takes one more pass at once, so a lease that lapsed mid-think (which broadcasts nothing) does not strand the turn. No protocol, migration or single-node behaviour change. ADR-0080 carries the amendment.
- The router's takeover reload (`rehydrateIfStale`, shared with `route()`) tracked in-flight reloads by game only: a slow reload begun for an earlier claim could finish after a later claim and mark a copy that predates another owner's moves as fresh. Reload debt now belongs to the claim that recorded it (a process-wide, never-reused claim number), and `prepareOwnership` re-checks the lease after reloading. Separately, `OwnershipRegistry` recorded a `SET NX` claim as continuing ownership whenever a failed renewal had left the game in its owned set, so a lease that lapsed unnoticed while another node advanced the game was re-claimed with no reload; a `SET NX` that creates the key is now always a new claim.
- Recovery without events: a replica that registered a bot game but does not own it re-checks every lease TTL, reloading its copy from the event log, so a game whose owner died is claimed and continued and a game whose `ended` broadcast was lost is unregistered. `RealtimeGateway` gains `onGameUnloaded` (the last session in a game left this node). The mover then marks the game as having no local player and takes a pass, dropping it only when a pass fails to own it (a join clears the mark): a non-owner lets go, a claim still in flight that succeeds keeps the game, a former owner lets go on its first failed claim, and the owner keeps the game for a player who reconnected elsewhere. Re-checks and their full replays therefore run only while someone on that replica is in the game, apart from retries after a pass that failed outright (Redis or the event log unreachable). A copy that already shows the game over unregisters before any ownership traffic, which covers joining a finished game on a non-owner. Remaining limit: a dead owner's game resumes after its lease expiry plus up to one re-check period, and only where someone is in the game on a surviving replica. The chart refuses a non-boolean `gateway.engineBot.enabled`, since a quoted `"false"` would leave the bot on.
...
- Tests: `engine-bot.test.ts` adds the ownership gate, post-engine checks, lapsed-lease re-run, re-check recovery and finished-game cleanup with a scripted gate; `engine-bot-multinode.integration.test.ts` proves the multi-replica behaviour on real Redis with the production router, registry and per-node Redis pub/sub (owner-only computation including duplicate broadcasts, ply-0 claim, takeover rehydration, ownership lost or lost-and-regained mid-think, an old reload landing after a later claim, a lease that lapsed unnoticed then re-claimed, a dead owner replaced with no further event, taking over a game that ended out of sight, session-leave release, terminal stop), the unit suite adds the session-leave rule's claim-in-flight cases (the claim succeeding and failing), former-owner and rejoin cases, and `realtime-gateway` tests the new hook. Its Redis clients keep production retry settings; a test-only preflight fails the file in seconds, by name, when Redis is unreachable. Fifteen deliberate defects were each introduced alone against the compiled test build, and each failed at least one test. `helm-snapshot-test.sh` pins the flag on, off, per strategy, absent from the search indexer, and the rest of the gateway env contract including the tournament reporter.
- `createRedisPubSub`'s subscriber connection no longer runs ioredis's ready check. A subscription made right after connecting (startup subscribers, a reconnect, and this suite's nodes, where it failed CI once) could switch the connection to subscriber mode before the check's `INFO`, which Redis refuses; ioredis then reset the connection and dropped whatever was published meanwhile. Pub/sub does not need the dataset-loaded check.
- Scope: `services/gateway` (engine bot, router ownership methods, ownership registry claim rule, subscriber connection options, wiring), `packages/realtime-gateway` (the optional `onGameUnloaded` hook), the Helm chart, snapshot test, and docs. BOT_AUTO_ANALYZE, ANTICHEAT_AUTO_ANALYZE, the tournament reporter, the search indexer, ratings, timed games and auth are untouched.

## M15 Increment 70 — Durable event-log-driven games projection (2026-09-26)

- Audit P0 "Game history / projection". Verified on `main` at `93cc29c`: only seek acceptance and bot games inserted a `games` row, and they did so at creation. Tournament games and other direct `GameCreated` appends had no row. `updateProgress` and `finish` had no callers, so every row stayed at `ply_count 0` with no result. `/v1/games/:id`, user history, the ended-game seek receipt guard, search and achievements all read that state.
- `PgGamesProjector` is now the single writer after creation (ADR-0147). Migration 0040 adds `game_events.xact_id` (the writing transaction, filled by a column default, so no append path changed), the `projection_checkpoints` row and `games_projection_failures`. Migration 0041 builds the checkpoint-order index online. A batch consumes only rows from transactions below `pg_snapshot_xmin(pg_current_snapshot())`, so a late-committing transaction is never skipped. `server_ts` is not used as a cursor. One transaction holds the checkpoint `FOR UPDATE SKIP LOCKED`, re-folds each touched game from its full stream through the pure `projectGameStream` (canonical `classifySpeed`, event timestamps), upserts `WHERE games.last_seq <= EXCLUDED.last_seq`, and advances the checkpoint in the same commit. Replicas take turns, replay is a no-op, and nothing regresses.
- A stream that cannot be folded (missing upcaster, impossible sequence, constraint violation) rolls back to a per-game savepoint. Transient database errors instead abort the batch without moving the checkpoint. It is recorded durably with exponential retry (at most hourly), logged, and never blocks other games. Seats that are not accounts project as NULL. A checkpoint above the cluster's transaction ids, as after a logical restore elsewhere, is detected and replayed from the origin.
- Runtime: every gateway with `DATABASE_URL` runs a `GamesProjectionWorker`: back to back while behind, a 1 s poll when idle, exponential backoff up to 30 s on failure, and an awaited in-flight batch on shutdown. Compose and Helm already provide the variable, so there is no deploy change. Pre-0040 streams are replayed by the first pass. `npm run games:rebuild` re-folds everything on demand and is safe beside live projection. Each rebuild page reads one REPEATABLE READ snapshot and leaves games the live projector has yet to reach to that projector, so a game that ends during a rebuild still wakes its consumers. After a logical restore it defers nothing. A page that keeps conflicting with live writes is redone game by game, so only a persistently conflicting game is reported.
- The search indexer and achievements worker now wake on a new `gamesProjectedEndedChannel()`, which is published once per game when its projected row becomes terminal, instead of the authority's earlier broadcast that raced the projection. The gateway starts projection after those workers subscribe, and on shutdown unsubscribes and drains them only after the in-flight batch. The unused, non-monotonic `start`, `updateProgress` and `finish` were removed from `GamesRepository` and its PostgreSQL implementation. `docs/DATABASE.md` §3.2, §4.2 and §6 now describe the asynchronous projector instead of append-time projection.
- Tests: `games-projection.test.ts` covers the fold, the corrupt-stream cases and the worker's scheduling, backoff and shutdown. `games-projector.integration.test.ts` runs on real PostgreSQL: direct, seek, bot and non-account creation; progress and endings with exactly-once ending reports; idempotent replay and rebuild; a rebuild deferring a game that is ending live, including an ending committed mid-page; a post-restore rebuild repairing everything; transient errors aborting a batch while data errors are recorded; no regression of a newer row; rebuild repair of missing and stale rows; the automatic backfill of pre-0040 data, including the append-only guard after the column rewrite; checkpoint-lock exclusion and two concurrent projectors converging; crash-before-commit atomicity and resume; the late-commit case that a timestamp cursor would skip; corrupt-stream isolation, backoff and recovery; checkpoint rewind. `games-projection.integration.test.ts` (api) serves a tournament-launched game's projected ending through `GET /v1/games/:id` and user history. Deliberate defects in the compiled projector were each caught by this suite; the PR description records the list.
- Limits (ADR-0147): the horizon is cluster-wide, so a long or idle-in-transaction session anywhere on the server delays projection but never loses work. Migration 0040 rewrites `game_events` once under an exclusive lock. Each touched game is re-folded in full. The post-projection wake is lossy pub/sub, so search and achievement recovery is still not durable, and achievements still need per-game award deduplication.
- Scope: persistence (migration, fold, projector, rebuild CLI, removal of the dead writers), realtime-gateway (channel), gateway wiring, api in-memory fake, tests, ADR-0147 and docs. Ratings, rating pools, leaderboards, readiness, first-move clock start, no-show and seek lifecycle, and autonomous flag expiry are untouched. Ratings are the follow-up that will consume this projection.

## M15 Increment 71 — Durable player readiness, first-move clock start and source-specific no-show (2026-09-26)

- Audit P0 "Timed-match lifecycle" (clock start and pregame abandonment). Verified on `main` at `deba3a8`: every game's clock was anchored at `GameCreated`, so White's first move was charged for the time before either player arrived; nothing recorded that a player had arrived (room presence is per-node and transient); and nothing ended a game nobody played, so an abandoned seek or tournament game stayed ongoing forever. ADR-0148 records the design.
- Source classification from the creation paths on `main`: seek acceptance and `DurableGameLauncher` create two-human games and now write `GameCreated.source` (`seek` or `tournament`). `POST /v1/games/bot` (human vs a first-party engine that never joins) and `GameAuthority.createGame` (used only by the e2e harness and tests) write no source and keep the original lifecycle unchanged. Stored games have no source and replay and continue exactly as before.
- Readiness: a new `PlayerReady` event, emitted by `Game.markReady` only for a sourced game that is ongoing, has no move and whose seat is not yet ready, so repeats are no-ops. After an authenticated seated join, `RealtimeGateway` routes a `ready` command to the game's owner through the command router, where it is serialized with moves and appended with `expectedSeq`; a losing append is re-routed (up to three attempts) against the reloaded log. Spectators, anonymous connections and other users' tokens never route it, and the wire decoder accepts neither `ready` nor `expireNoShow`. Readiness survives disconnects and restarts because it is only ever read from the log.
- Clock: a sourced game has no clock anchor until its first move, which consumes no chess time and anchors the opponent's clock at the move's server timestamp. The first move is refused (`not_ready`) until both seats are ready; the domain enforces it and the authority reports the code. Replay uses stored `moveTimeMs`, so it never reads the wall clock.
- No-show: `NoShowExpiryWorker` runs on every gateway with `DATABASE_URL`. Each pass reads a keyset page of possibly-due games from the games projection through the partial index `games_pregame_pending_idx`, re-decides each from the event log, and routes `expireNoShow` as `NO_SHOW_ACTOR` to the owner. Seek games end `'*'`/`no_show` 60 s after creation if no move was played, whoever was ready; tournament games end after 5 minutes as a forfeit win for the one ready player or a winnerless double forfeit, and wait indefinitely when both are ready. `NO_SHOW_SEEK_MS`, `NO_SHOW_TOURNAMENT_MS` and `NO_SHOW_SCAN_MS` are validated positive integers. The owner's command lock plus the log's sequence check make a first move and an expiry mutually exclusive and make a second expiry a no-op. A claim taken only to expire a game with no local room is released afterwards. Shutdown awaits the in-flight pass before Redis and the pool close.
- Tournaments: the reporter maps a winnerless `no_show` ending to the existing `double_forfeit` result, so it never reaches the aborted-game relaunch; a `no_show` with a winner records like any decisive result. Committed-terminal-event recovery (ADR-0144) is unchanged. No tournament domain change was needed.
- Projection and migrations: `games.source` is projected from the event log; `PlayerReady` advances `last_seq` but not the ply count; a no-show ending projects result, `termination = 'no_show'` and `ended_at`, which also makes the ended-game seek receipt guard truthful. Migration 0042 adds the `no_show` termination and `games.source` with a `NOT VALID` check, 0043 validates it online, and 0044 builds the pending index concurrently. The pre-0040 projector test now migrates through 0041 for its count assertion.
- Protocol and UI: `StateView.ready` (null for games without a source), a `ready` broadcast on the game channel (replayed on resume), and the `not_ready` reject code. The web client reads a missing or malformed `ready` as none, merges broadcasts monotonically, keeps the board closed while a seat is not ready, and shows waiting and no-show states. No visual change.
- CI: the gateway-service job now also has a pgvector PostgreSQL service and `DATABASE_URL`, because the new routed-replica suite needs both Redis and PostgreSQL; `scripts/ci-local.mjs` requires both for that job.
- Tests: domain (`pregame-lifecycle.test.ts`), authority and gateway (`readiness.test.ts`), web (`readiness.test.ts`), API (`pregame-no-show.test.ts`), persistence fold and real-PostgreSQL migrations, index use, projection, keyset scan, pre-0042 upgrade and log race (`pregame-projection.test.ts`, `pregame-no-show.integration.test.ts`), the worker hermetically (`no-show-expiry.test.ts`), and two routed replicas on real Redis and PostgreSQL (`no-show-expiry.integration.test.ts`): duplicate scans, cross-replica readiness, crash after readiness, all tournament outcomes, both-ready waiting and the uncharged first move, first move racing expiry, restart while overdue, and the reporter recording a double forfeit without relaunch.
- Limits (ADR-0148): expiry lags by at most a scan interval plus projection lag; configuration changes apply to waiting games; replicas of the previous release cannot fold streams containing `PlayerReady`, so migrations run first and every gateway replica is replaced in one rollout; a present player may still abort before the second move, which keeps the existing tournament relaunch. Autonomous in-play flag expiry is the next increment; ratings are untouched.
