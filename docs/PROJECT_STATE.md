...
- Residual threat, stated plainly: password guessing spread across many independent source addresses is limited per address, at 5 guesses per handle per 15 minutes per address, and cannot be hard-blocked globally. Blocking it globally would either reintroduce a victim-targetable lockout or need a separate proof such as a challenge. That adaptive, secondary-proof design is a separate security enhancement, not part of this fix.
