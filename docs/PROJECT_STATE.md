...
- An existing usable password-reset token is never replaced by another unauthenticated request. Under a stable PostgreSQL user-row lock, at most one live token is issued per account; repeated requests send no additional email. Only a definitive provider rejection or throttle discards the exact unsent token; timeout or ambiguous failure leaves it usable. Expiry or single-use consumption permits a future token. Raw tokens are never stored.
- New deterministic in-memory and real-PostgreSQL two-replica tests cover distributed requests, one live hashed token and email, rotating attacker addresses, unchanged link validity, provider outcomes, public response parity, and shutdown draining. Existing recovery tests continue to cover expiry, single use, password update, and full-session revocation. ADR-0146 records the updated security contract. This increment does not redesign login step-up, email verification, or other rate-limit surfaces.
