...
NETWORK_POLICY_IPBLOCK_COUNT=$(yq 'select(.kind=="NetworkPolicy") | .spec.ingress[].from[] | select(.ipBlock != null) | .ipBlock.cidr' "$TMPDIR/default.yaml" 2>/dev/null | grep -vx -- '---' | grep -c . || true)
